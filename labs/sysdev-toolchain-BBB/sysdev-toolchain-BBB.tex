\subchapter{Extra Lab: Building a cross-compiling toolchain using Crosstool-NG}
  {Objective: Learn how to configure and compile your own cross-compiling toolchain for the uClibc C
  library}
...
This generates the \code{.config} file in the source directory, but we will replace it with
something useful.

Copy the \code{uclibc-cortex-a8.config} and the \code{uClibc-0.9.33.config} files from the toolchain made easy lab, to the crosstool-NG top directory, and then copy \code{uclibc-cortex-a8.config} to the \code{.config} file.
...
\begin{itemize}
\item Set \code{binutils version} to \code{2.23.1}.
\end{itemize}

In \code{Debug facilities}:
\begin{itemize}
\item Enable \code{gdb}, \code{strace}.
\item Remove the other options (\code{ltrace}, \code{dmalloc} and \code{duma}).
 Note that building \code{ltrace} will fail.
