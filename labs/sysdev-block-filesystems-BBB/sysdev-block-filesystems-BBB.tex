...
You need to copy the new kernel to \code{/tftpboot} and reinstall 
the modules into \nfsroot.
...
Using \code{cfdisk}
\footnote{When you have initialized a partition and want to add more, you don't have to specify headers and sectors again. Just run \code{cfdisk /dev/sdx}}.
...
Before creating the filesystem, you should create \code{${HOME}/rootfs/boot},
...
the MMC card (the partition called \code{boot}), and edit the \code{uSetup.txt}
