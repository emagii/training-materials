...
Go to the \code{$HOME/labs/sysdev/tinysystem/} directory.
...
\footnotesize
\begin{verbatim}
<install-dir>/labs/sysdev/tinysystem/nfsroot 192.168.0.100(rw,no_root_squash,no_subtree_check)
\end{verbatim}
\normalsize
...
Add the following U-Boot commands to your uEnv.txt to do so, {\bf in just 1 line}
(Caution: in \code{ttyO2} below, it's the capital letter \code{O}, like in
{\bf O}MAP and not the number zero):

\begin{verbatim}
console=console=ttyO2,115200
ipaddr=192.168.0.100
serverip=192.168.0.1
nfsroot=<install-dir>/labs/sysdev/tinysystem/nfsroot
bootargs=${console} root=/dev/nfs ip=${ipaddr}
   nfsroot=${serverip}:${nfsroot} rw
\end{verbatim}
