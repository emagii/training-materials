...
Normally, \code{u-boot} executed the \code{bootcmd} which reads the
\code{uEnv.txt} which modifies the boot. The problem with this approach, is
that you cannot stop the boot.

Our patch (which is not standard) will start by reading the \code{uSetup.txt}
before \code{bootcmd} is executed, so you have a chance to stop the boot,
by pressing{\bf return}.

You should create the file \code{uSetup.txt} looking like:
...
Copy \code{uSetup.txt} it to the FAT partition and also create an empty \code{uEnv.txt} in the same place.
...
www.kwewww.kernel.Press the \code{<return>} button before the counter expires, to enter the U-Boot shell.
